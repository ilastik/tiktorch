...
class MrRobot:
    ...
    def __init__(self, path_to_config_file, strategy_set, devices: List[str]) -> None:

        assert torch.cuda.device_count(
        ) == 1, f"Device count is {torch.cuda.device_count()}"
        # start the server
        ...
        if self.base_config["data_dir"]["raw_data_base_folder"].endswith(".h5"):
            self.raw_data_file = h5py.File(
                self.base_config["data_dir"]["raw_data_base_folder"], 'r')
            self.labelled_data_file = h5py.File(
                self.base_config["data_dir"]["labelled_data_base_folder"], 'r')
            self.validation_raw_file = h5py.File(
                self.base_config["data_dir"]["validation_raw_base"], 'r')
            self.validation_label_file = h5py.File(
                self.base_config["data_dir"]["validation_label_base"], 'r')
        else:
            self.raw_data_file = z5py.File(
                self.base_config["data_dir"]["raw_data_base_folder"], 'r')
            self.labelled_data_file = z5py.File(
                self.base_config["data_dir"]["labelled_data_base_folder"], 'r')
            self.validation_raw_file = h5py.File(
                self.base_config["data_dir"]["validation_raw_base"], 'r')
            self.validation_label_file = h5py.File(
        ...
        image_shape = self.raw_data_file[self.base_config["data_dir"]
                                         ["path_to_raw_data"]].shape
        validation_data_shape = self.validation_raw_file[self.base_config["data_dir"]
                                                         ["validation_raw"]].shape
        print(image_shape)
        self.block_list = tile_image(
            image_shape, self.base_config["training"]["training_shape"])
        self.validation_block_list = tile_image(
            validation_data_shape, self.base_config["training"]["training_shape"])
        print("number of patches: %s" % len(self.block_list))
        print()

        strategy_objects = []
        for(strategy in strategy_set):
            if strategy == "VideoLabelling":
                videolabelling = VideoLabelling(self.base_config[strategy_params]["loss_fn"], self.base_config["class_dict"], self.raw_data_file, self.labelled_data_file, self.base_config["data_dir"],
                                                self.base_config["strategy_params"]["labelling_strategy"], self.base_config[strategy_params]["annotatoin_percent"], self.base_config["training"]["training_shape"])
                strategy_objects.append(videolabelling)
            else:
                new_strategy_object = strategy(self.base_config[strategy_params]["loss_fn"], self.base_config["class_dict"], self.raw_data_file, self.labelled_data_file,
                                               self.base_config["data_dir"], self.base_config["strategy_params"]["labelling_strategy"], self.base_config[strategy_params]["annotatoin_percent"])
                strategy_objects.append(new_strategy_object)

        self.strategy = StrategyAbstract(self.new_server, strategy_objects)
        ...
        mr_robot_folder = os.path.dirname(
            os.path.dirname(os.path.abspath(__file__)))
        self.tensorboard_writer = SummaryWriter(logdir=os.path.join(
            mr_robot_folder, "tests", "robot", "robo_logs", "strat_classwise"))
        self.patch_id = dict()
    ...
    def _load_model(self):
        ...
        if self.base_config["model_dir"]["path_to_folder"].endswith(".zip"):
            archive = zipfile.ZipFile(
                self.base_config["model_dir"]["path_to_folder"], "r")
            model_file = archive.read(
                self.base_config["model_dir"]["path_in_folder_to_model"])
            binary_state = archive.read(
    ...
    def _predict(self):
        ...
        for block in self.block_list:
            ...
            is_patch_already_in_training_flag = self.assign_id(block, x)
            if(is_patch_already_in_training_flag == True):
                continue
            # self.patch_id[block[0].start] = x
            # pred_output = self.new_server.forward(NDArray(self.raw_data_file[path_to_input][block]))
            prediction_list.append(
                batch_maker.submit(self.new_server.forward, NDArray(
                    self.raw_data_file[path_to_input][block], x))
            )
            x += 1
        ...
        for prediction in cf.as_completed(prediction_list):
            ...
            self.strategy.update_state(
                prediction.result().as_numpy(
                ), self.labelled_data_file[path_to_label][block], block, False
            )
    ...
    def validate(self):
        ...
        for block in self.validation_block_list:
            validation_list.append(
                batch_maker.submit(self.new_server.forward, NDArray(
                    self.validation_raw_file[path_to_input][block], x))
            )
            x += 1
        for prediction in cf.as_completed(validation_list):
            ...
            self.strategy.update_state(
                prediction.result().as_numpy(
                ), self.validation_label_file[path_to_label][block], block, True
            )
    ...
    def _run(self):
        ...
        while not self.stop():
            ...
            self.stats["training_iterations"].append(
                curr_model_state.num_iterations_done)
            # self.stats["training_loss"].append(curr_model_state.loss)
            self.stats["number_of_patches"].append(
                (self.iterations_done - 1) * batch_size)
    ...
    def _add(self, new_data_batch):
        ...
        self.new_server.update_training_data(
            NDArrayBatch(new_inputs), NDArrayBatch(new_labels))
        print("addition done!!")
    ...
    def write_to_tensorboard(self):
        ...
        if(self.iterations_done == 1):
            ...
            for key, value in self.patch_id.items():
                if value == -1:
                    ...
                    block = tuple([slice(key[i], key[i] + training_shape[i])
                                   for i in range(len(training_shape))])
                    print(block)
            ...
            for block in training_block:
                ...
                train_prediction_list.append(
                    batch_maker.submit(
                        self.new_server.forward, NDArray(image, x))
                )
                x += 1
            print()
            ...
            for prediction in cf.as_completed(train_prediction_list):
                ...
                train_loss += criterion_class_obj(
                    torch.from_numpy(pred_output.astype(np.float32)), torch.from_numpy(
                ...
                conf_matrix += get_confusion_matrix(
                    pred_output, target, list(self.base_config["class_dict"].keys()))
        ...
        training_confusion_matrix = plot_confusion_matrix(
            training_confusion_matrix, self.base_config["class_dict"])
