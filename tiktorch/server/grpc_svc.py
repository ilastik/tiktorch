...
from tiktorch.server.session import start_session_process
...
class InferenceServicer(inference_pb2_grpc.InferenceServicer, inference_pb2_grpc.FlightControlServicer):
    ...
    def CreateModelSession(
        ...
        try:
            _, client = start_session_process(model_zip=request.model_blob.content, devices=[d.id for d in lease.devices])
        except Exception:
            lease.terminate()
            raise
