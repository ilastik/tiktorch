...
def test_eval_torchscript_model_zip_predict(
    ...
    with ZipFile(pybio_unet2d_torchscript_bytes) as zf:
        pipeline = eval_model_zip(zf, devices=["cpu"], cache_path=cache_path, preserve_batch_dim=True)
...
def test_eval_model_zip_metadata_no_batch_dim(
    ...
    with ZipFile(pybio_unet2d_torchscript_bytes) as zf:
        pipeline = eval_model_zip(zf, devices=["cpu"], cache_path=cache_path, preserve_batch_dim=False)
